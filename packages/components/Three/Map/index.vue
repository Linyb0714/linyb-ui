...
onMounted(() => {
  ...
  /** 遍历地图数据 */
  const mapForData = () => {
    // const mapObj = jsonData.features.filter(item => item.properties.name === '广东省');
      mapAreaData = [];
    jsonData.features.forEach(mapObj => {
      processData(mapObj); // 处理地图数据
    })
    
  }
  // 处理地图数据
  const processData = (_mapArea: any) => {
    console.log('获取地图数据', _mapArea);
    
    mapAreaData = _mapArea.geometry.coordinates[0];
    // 数据 从 经纬度-> 像素坐标-> 三维坐标
    mapAreaData = mapAreaData.map((item: any) => {
      const two = latlng2px(item);
      const three = new THREE.Vector3(two[0], 0, two[1]);
      return three;
    })
    createMap(mapAreaData); // 创建地图块
    createEdge(mapAreaData); // 创建边缘线
  }

  // 创建地图块
  const createMap = (_mapArea: any) => {
    const shape = new THREE.Shape();
    shape.moveTo(_mapArea[0].x, _mapArea[0].z);// 移动到第一个点
    for (let i = 1; i < _mapArea.length; i++) {
      shape.lineTo(_mapArea[i].x, _mapArea[i].z);// 连线
    }
    shape.lineTo(_mapArea[0].x, _mapArea[0].z);// 闭合
  ...
  // 创建边缘线
  const createEdge = (_mapArea: any) => {
    // 创建曲线(数据, 是否闭合, 曲线类型, 张力)
    const curve = new THREE.CatmullRomCurve3(_mapArea, true, 'catmullrom', 0);
    // 创建管状几何体(曲线, 管道半径, 管道分段, 管道圆周分段, 是否闭合)
    const geometry = new THREE.TubeGeometry(curve, Math.round(_mapArea.length * 0.5), 0.01, 8, true);
